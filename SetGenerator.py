...
def setGenerator(write,filepath):
    ...
    if write:
        ...
        songcsv.write("Title, ID, Acousticness, Danceability, Energy, Instrumentalness, Liveness, Loudness, Speechiness, Valence, Tempo, Popularity\n")

    # Generating song features for each song given
    features = []
    for id,name,popularity in ids:
        fts = spotify.audio_features(id)[0]
        try:
            flist = [name,id,fts['acousticness'],fts['danceability'],fts['energy'],fts['instrumentalness'],fts['liveness'],2**((fts['loudness'])/6),fts['speechiness'],fts['valence'],fts['tempo']/220,popularity/100]
            features.append(flist)
        except:
            pass
        if write:
            songcsv.write(flist[0] + ',' + flist[1] + ',' + str(flist[2]) + ',' + str(flist[3]) + ',' + str(flist[4]) + ',' + str(flist[5]) + ',' + str(flist[6]) + ',' + str(flist[7]) + ',' + str(flist[8]) + ',' + str(flist[9]) + ',' + str(flist[10]) + ','+ str(flist[11]) + '\n')
    if write:
        songcsv.close()
